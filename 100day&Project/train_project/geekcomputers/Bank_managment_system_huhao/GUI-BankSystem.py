...
def Cr_Amt(accnt, name):
    ...
    l_title = tk.Message(creditwn, text="JUIT BANK", relief="raised", width=2000, padx=600, pady=0, fg="white",
...
def De_Amt(accnt, name):
    ...
    l_title = tk.Message(debitwn, text="JUIT BANK", relief="raised", width=2000, padx=600, pady=0, fg="white",
                         bg="black", justify="center", anchor="center")
    l_title.config(font=("Courier", "50", "bold"))
    l_title.pack(side="top")
...
def disp_tr_hist(accnt):
    ...
    l_title = tk.Message(disp_wn, text="JUIT BANK", relief="raised", width=2000, padx=600, pady=0, fg="white",
                         bg="black", justify="center", anchor="center")
    l_title.config(font=("Courier", "50", "bold"))
    l_title.pack(side="top")
...
def logged_in_menu(accnt, name):
    ...
    fr1 = tk.Frame(rootwn)
    fr1.pack(side="top")
    l_title = tk.Message(rootwn, text="WELCOME TO\nJUIT BANK", relief="raised", width=2000, padx=600, pady=0,
...
def log_in(master):
    ...
    l_title = tk.Message(loginwn, text="JUIT BANK", relief="raised", width=2000, padx=600, pady=0, fg="white",
                         bg="black", justify="center", anchor="center")
    l_title.config(font=("Courier", "50", "bold"))
    l_title.pack(side="top")
...
def Create():
    ...
    l_title = tk.Message(crwn, text="JUIT BANK", relief="raised", width=2000, padx=600, pady=0, fg="white", bg="black",
    ...
    b.pack(side="top")
    crwn.bind("<Return>", lambda x: write(crwn, e1.get().strip(), e2.get().strip(), e3.get().strip()))  # 获得文本框里的内容
    return


def Main_Menu():
    rootwn = tk.Tk()
    rootwn.geometry("1600x500")  # 长宽
    rootwn.title("JUIT Bank")  # window名字
    rootwn.configure(background='orange')  # 背景颜色
    fr1 = tk.Frame(rootwn)  # 定义主框架
    fr1.pack(side="top")
    bg_image = tk.PhotoImage(file="pile1.gif")
    x = tk.Label(image=bg_image)
    x.place(y=-400)
    l_title = tk.Message(text="WELCOME TO\nJUIT BANK", relief="raised", width=2000, padx=600, pady=0, fg="white",
                         bg="black", justify="center", anchor="center")
    l_title.config(font=("Courier", "50", "bold"))
    l_title.pack(side="top")
    ...
    rootwn.mainloop()  # 让窗口不断刷新，让点击事件能够响应
